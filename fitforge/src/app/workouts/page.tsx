            ...
            <AnimatePresence>
                {restTimerActive && (
                    <motion.div
                        ...
                        className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-zinc-900/95 backdrop-blur-md border border-emerald-500/30 rounded-2xl px-6 py-4  flex items-center gap-4"
                    >
                        ...
                        <Button
                            ...
                            className="text-muted-foreground hover:text-zinc-50"
                        >
                            <X size={16} />
                        </Button>
                    </motion.div>
                ...
                {workoutActive && currentEx && (
                    ...
                    >
                        <motion.div
                            ...
                            animate={{ scale: 1, y: 0 }}
                            exit={{ scale: 0.9, y: 30 }}
                            className="bg-zinc-900 border border-zinc-800/50 rounded-2xl p-6 w-full max-w-md"
                        >
                            ...
                            <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-zinc-80020 flex items-center justify-center mx-auto mb-4">
                            ...
                            <div className="space-y-2">
                                {currentEx.instructions.slice(0, 2).map((step, i) => (
                                    <p key={i} className="text-xs text-muted-foreground"><span className="text-orange-400 font-mono">{i + 1}.</span> {step}</p>
                                ))}
                            </div>
                            <div className="flex gap-3 mt-5">
                                <Button variant="ghost" size="sm" onClick={handleGuidedSkip} className="flex-1 border border-zinc-800/50 text-muted-foreground">
                ...
                {showCompletionModal && (
                    ...
                    >
                        <motion.div
                            ...
                            animate={{ scale: 1, y: 0 }}
                            exit={{ scale: 0.8, y: 30 }}
                            className="bg-zinc-900 border border-emerald-500/30 rounded-2xl p-6 w-full max-w-sm text-center"
                        >
                            <motion.div
                            ...
                            <div className="flex gap-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 mb-5">
                                ...
                                <div className="flex-1 border-l border-zinc-800/50">
                                    <p className="text-xs text-muted-foreground">XP Earned</p>
                                    <p className="text-lg font-bold text-orange-400">+{guidedExercises.length * 15}</p>
                                </div>
                                <div className="flex-1 border-l border-zinc-800/50">
            ...
            <Tabs defaultValue="planner" className="space-y-4">
                ...
                <TabsContent value="planner" className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={resetWeek} className="text-xs border-zinc-800/50">
                        ...
                        {weeklyPlan.map((day, dayIndex) => {
                            ...
                            return (
                                <motion.div key={day.day} custom={dayIndex} initial="hidden" animate="visible" variants={fadeUp}>
                                    <Card className={`glass-card border-zinc-800/50 ${isToday ? 'ring-1 ring-orange-500/30' : ''}`}>
                                                    ...
                                                    {!day.isRestDay && (
                                                        <>
                                                            ...
                                                            <Button
                                                                ...
                                                                className="text-[11px] h-7 px-2 text-orange-400 hover:text-cyan-300 hover:bg-orange-500/10 flex items-center gap-1"
                                                            >
                                                                <Play size={11} /> Start
                                                            </Button>
                                                        </>
                                                        ...
                                                        return (
                                                            <motion.button
                                                                ...
                                                                className={`group w-full flex items-center gap-3 p-2.5 rounded-lg text-left transition-all ${isDone
                                                                    ...
                                                                    : 'bg-zinc-900/50 border border-zinc-800/50 hover:border-zinc-800/50'
                                                                    }`}
                                                            >
                                                                {isDone ? (
                                                                    ...
                                                                    <motion.div key="undone" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}>
                                                                        <Circle size={16} className="text-muted-foreground flex-shrink-0 transition-colors duration-200 group-hover:text-orange-400" />
                                                                    </motion.div>
                                                                )}
                                                                {(() => {
                                                                    const CatIcon = ex.icon;
                                                                    return (
                                                                        <div className="w-8 h-8 rounded-lg bg-zinc-900/50 flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
                                                                            <CatIcon size={15} className="text-orange-400" />
                                                                        </div>
                                                                    );
                                                                })()}
                ...
                <TabsContent value="exercises" className="space-y-4">
                    ...
                    <div className="relative">
                        ...
                        <Input
                            ...
                            className="pl-9 bg-zinc-900/50 border-zinc-800/50"
                        />
                    </div>
                        ...
                        {categories.map((cat) => {
                            ...
                            return (
                                <Button
                                    ...
                                    className="text-xs flex-shrink-0 border-zinc-800/50 flex items-center gap-1.5"
                                >
                                    <CatIcon size={12} />
                                    {cat.name}
                                </Button>
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                        {filteredExercises.map((ex, i) => (
                            <motion.div key={ex.id} custom={i} initial="hidden" animate="visible" variants={fadeUp}>
                                <Card
                                    className="glass-card border-zinc-800/50 hover:border-zinc-800/50 transition-all cursor-pointer group"
                                    onClick={() => setSelectedExercise(selectedExercise?.id === ex.id ? null : ex)}
                                >
                                    <CardContent className="p-4">
                                        <div className="flex items-start gap-3">
                                            <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-80020 flex items-center justify-center flex-shrink-0">
                                                <ex.icon size={24} className="text-orange-400" />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                            ...
                                            {selectedExercise?.id === ex.id && (
                                                ...
                                                >
                                                    <div className="mt-4 pt-4 border-t border-zinc-800/50 space-y-3">
                                                        <div>
                                                            <h4 className="text-xs font-medium text-orange-400 mb-1.5">Instructions</h4>
                                                            <ol className="space-y-1">
                                                                {ex.instructions.map((step, idx) => (
                                                                    <li key={idx} className="text-xs text-muted-foreground flex gap-2">
                                                                        <span className="text-orange-400/60 font-mono">{idx + 1}.</span>
                                                                        {step}
                                                                    </li>
                                                                ))}
                                                            </ol>
